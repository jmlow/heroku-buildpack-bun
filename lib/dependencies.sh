...
bun_install() {
	...
	monitor "bun-install" bun install 2>&1
}
...
bun_prune_devdependencies() {
	...
	if [[ ${BUN_ENV} == "test" ]]; then
		echo "Skipping because BUN_ENV is 'test'"
		meta_set "skipped-prune" "true"
		return 0
	elif [[ ${BUN_ENV} != "production" ]]; then
	...
	monitor "bun-prune" bun install --production 2>&1
	meta_set "skipped-prune" "false"
}
